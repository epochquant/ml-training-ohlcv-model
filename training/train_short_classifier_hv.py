...
EXTRA_FEATURE_COLS = ['atr_pct', 'overextension', 'vol_ratio', 'upper_wick_ratio']


class ShortReversalDatasetHV(Dataset):
    """High-volatility counterpart of training.train_short_classifier.ShortReversalDataset.

    Same windowing/z-score normalization for the backbone input, plus the
    Area C extra features (src/data/label_short_signals_hv.py::compute_indicators_hv)
    read at the final historical candle and fused into the classifier head.
    training/train_short_classifier.py itself is untouched.
    """
    ...
    def __init__(self, df: pd.DataFrame, seq_len: int = 400, clip: float = 5.0):
        ...
        for col in feature_cols + time_cols + EXTRA_FEATURE_COLS:
            if col not in df.columns:
                df[col] = 0.0

        self.features = df[feature_cols].values.astype(np.float32)
        self.time_features = df[time_cols].values.astype(np.float32)
        self.extra_features = df[EXTRA_FEATURE_COLS].values.astype(np.float32)
    ...
    def __getitem__(self, idx):
        ...
        extra = self.extra_features[end_idx - 1].copy()
...
def train_epoch(model, tokenizer, dataloader, optimizer, criterion, device, epoch, total_epochs, log_interval=10):
    ...
    for batch_idx, (x, x_stamp, extra, y) in enumerate(dataloader):
        x = x.to(device)
        ...
        optimizer.zero_grad()
        logits = model(s1_ids, s2_ids, extra, stamp=x_stamp)
        ...
        if (batch_idx + 1) % log_interval == 0 or (batch_idx + 1) == len(dataloader):
            ...
            print(f"[Epoch {epoch}/{total_epochs}, Step {batch_idx+1}/{len(dataloader)}] "
...
def evaluate(model, tokenizer, dataloader, criterion, device, threshold: float = 0.5):
    ...
    with torch.no_grad():
        for x, x_stamp, extra, y in dataloader:
            x = x.to(device)
            x_stamp = x_stamp.to(device)
            extra = extra.to(device)
            y = y.to(device)
...
def extract_dataset_features(dataloader, model, tokenizer, device):
    ...
    with torch.no_grad():
        for batch_idx, (x, x_stamp, extra, y) in enumerate(dataloader):
            ...
            features = model.extract_features(s1_ids, s2_ids, stamp=x_stamp)

            all_features.append(features.cpu())
            all_extra.append(extra.cpu())
            all_labels.append(y.cpu())

            if (batch_idx + 1) % 50 == 0 or (batch_idx + 1) == total_steps:
                print(f"  [Extraction Progress] Step {batch_idx+1}/{total_steps}")

    return torch.cat(all_features, dim=0), torch.cat(all_extra, dim=0), torch.cat(all_labels, dim=0)
...
def train_epoch_features(model, dataloader, optimizer, criterion, device, epoch, total_epochs, log_interval=10):
    ...
    for batch_idx, (features, extra, y) in enumerate(dataloader):
        features = features.to(device)
        extra = extra.to(device)
        y = y.to(device)

        optimizer.zero_grad()
        fused = torch.cat([features, extra], dim=-1)
...
def evaluate_features(model, dataloader, criterion, device, threshold: float = 0.5):
    ...
    with torch.no_grad():
        for features, extra, y in dataloader:
            features = features.to(device)
            extra = extra.to(device)
            y = y.to(device)
...
def main():
    parser = argparse.ArgumentParser(description="Train the high-volatility Kronos Short Reversal Classifier")
    parser.add_argument('--dataset', type=str, required=True, help="Path to labeled CSV dataset (label_short_signals_hv.py output)")
    ...
    parser.add_argument('--save_dir', type=str, default="./output_models_hv", help="Directory to save model weights")
    ...
    parser.add_argument('--gcs_output_dir', type=str, default="", help="GCS URI to upload output model (e.g. gs://bucket/high-volatility/short-models/)")
    ...
    if args.dataset.startswith("gs://"):
        ...
        local_dataset = "/tmp/dataset_hv.csv"
        fs = gcsfs.GCSFileSystem()
    ...
    if args.pretrained_kronos.startswith("gs://"):
        ...
        if fs.exists(gcs_clean):
            ...
            local_kronos = "/tmp/pretrained_kronos_hv"
    ...
    if args.pretrained_tokenizer.startswith("gs://"):
        ...
        if fs.exists(gcs_clean):
            ...
            local_tokenizer = "/tmp/pretrained_tokenizer_hv"
    ...
    train_dataset = ShortReversalDatasetHV(train_df, seq_len=400)
    val_dataset = ShortReversalDatasetHV(val_df, seq_len=400)
    ...
    model = HighVolatilityShortClassifier(
        kronos_base, d_model=kronos_base.d_model, dropout=0.2,
        freeze_backbone=freeze_backbone, n_extra_features=len(EXTRA_FEATURE_COLS)
    ).to(device)
    ...
    best_save_path = os.path.join(args.save_dir, "kronos_short_classifier_hv.pt")
    latest_save_path = os.path.join(args.save_dir, "kronos_short_classifier_hv_latest.pt")
    best_f1 = -1.0

    if freeze_backbone:
        train_features, train_extra, train_labels = extract_dataset_features(train_loader, model, tokenizer, device)
        val_features, val_extra, val_labels = extract_dataset_features(val_loader, model, tokenizer, device)

        train_feat_dataset = TensorDataset(train_features, train_extra, train_labels)
        val_feat_dataset = TensorDataset(val_features, val_extra, val_labels)
    ...
    print("\nStarting Training (High-Volatility)...")
    print("=" * 70)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if f1 >= best_f1:
            ...
            print(f"  [*] Saved new best HV model checkpoint to {best_save_path}")
            if args.gcs_output_dir:
