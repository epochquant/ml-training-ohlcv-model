...
from model.high_volatility import HighVolatilityTokenizer, HighVolatilityKronos
from model.high_volatility.model import load_hv_kronos_from_base
...
def create_dataloaders(config: dict, rank: int, world_size: int):
    """Same shape as training.train_predictor.create_dataloaders, but backed
    by HighVolatilityDataset (Area A normalized windows + Area B regime
    vectors)."""
...
def train_model(model, tokenizer, device, config, save_dir, logger, rank, world_size):
    ...
    for epoch_idx in range(config['epochs']):
        ...
        for i, (batch_x, batch_x_stamp, batch_regime) in enumerate(train_loader):
            batch_x = batch_x.to(device, non_blocking=True)
            batch_x_stamp = batch_x_stamp.to(device, non_blocking=True)
            batch_regime = batch_regime.to(device, non_blocking=True)
            ...
            if rank == 0 and logger:
                lr = optimizer.param_groups[0]['lr']
                logger.log_metric('train_hv_predictor_loss_batch', loss.item(), step=batch_idx_global)
                logger.log_metric('train_hv_S1_loss_each_batch', s1_loss.item(), step=batch_idx_global)
                logger.log_metric('train_hv_S2_loss_each_batch', s2_loss.item(), step=batch_idx_global)
                logger.log_metric('hv_predictor_learning_rate', lr, step=batch_idx_global)
        ...
        with torch.no_grad():
            for batch_x, batch_x_stamp, batch_regime in val_loader:
                batch_x = batch_x.to(device, non_blocking=True)
                batch_x_stamp = batch_x_stamp.to(device, non_blocking=True)
                batch_regime = batch_regime.to(device, non_blocking=True)
        ...
        if rank == 0:
            ...
            if logger:
                logger.log_metric('val_hv_predictor_loss_epoch', avg_val_loss, epoch=epoch_idx)

            if avg_val_loss < best_val_loss:
                best_val_loss = avg_val_loss
                save_path = f"{save_dir}/checkpoints/best_model"
                model.module.save_pretrained(save_path)
                print(f"Best HV predictor model saved to {save_path} (Val Loss: {best_val_loss:.4f})")
...
def main(config: dict):
    ...
    tokenizer = HighVolatilityTokenizer.from_pretrained(config['finetuned_tokenizer_path'])
    tokenizer.eval().to(device)

    model = load_hv_kronos_from_base(config['pretrained_predictor_path'], n_regime_features=config['n_regime_features'])
    model.to(device)
    model = DDP(model, device_ids=[local_rank], find_unused_parameters=False)

    if rank == 0:
        print(f"HV Predictor Model Size: {get_model_size(model.module)}")

    dt_result = train_model(model, tokenizer, device, config, save_dir, comet_logger, rank, world_size)

    if rank == 0:
        master_summary['final_result'] = dt_result
        with open(os.path.join(save_dir, 'summary.json'), 'w') as f:
            json.dump(master_summary, f, indent=4)
        print('HV predictor training finished. Summary file saved.')
        if comet_logger:
            comet_logger.end()
