...
def main():
    ...
    container_image = get_env_var(
        ...
        f"{registry_region}-docker.pkg.dev/{project_id}/{repository_name}/ohlcv-model-training:latest",
    )
    ...
    if args.non_interactive:
        ...
        region = args.region or prompt("3. Primary GCP Region (e.g., europe-west4, us-central1)", default=get_env_var("GCP_REGION", "europe-west4"))
        ...
        fallback_input = prompt("6. Fallback Regions (Global Multi-Region Diversity)", default=args.fallback_regions or DEFAULT_FALLBACK_STR)
        fallback_regions = [r.strip() for r in fallback_input.split(",") if r.strip()]
